...
class Location {
  ...
  init(filename: String) {
    guard
      let properties = Location.plist(filename) as? [String: Any],
      let boundaryPoints = properties["boundary"] as? [String]
      else { return }

    midCoordinate = Location.parseCoord(dict: properties, fieldName: "midCoord")
    overlayTopLeftCoordinate = Location.parseCoord(
      dict: properties,
      fieldName: "overlayTopLeftCoord")
    overlayTopRightCoordinate = Location.parseCoord(
      dict: properties,
      fieldName: "overlayTopRightCoord")
    overlayBottomLeftCoordinate = Location.parseCoord(
      dict: properties,
